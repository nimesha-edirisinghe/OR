...
import { neutral_400, ocean_blue_500, ocean_blue_600, ocean_blue_700 } from 'theme/colors';
...
const AppTimePicker: FC<Props> = ({ onTimeSelect, time, setIsClockVisible }) => {
  ...
  const onUpdateHours = (value: string) => {
    if (
      ((amPm === 'PM' || amPm === 'AM') && isNumeric(value) && parseInt(value) <= 12) ||
      value === ''
    ) {
      onTimeSelect(parseInt(value) || 0, parseInt(minute) || 0, true, amPm);
    }
  };
  const onUpdateMinutes = (value: string) => {
    ((isNumeric(value) && parseInt(value) <= 59) || value === '') &&
      onTimeSelect(parseInt(hour) || 0, parseInt(value) || 0, true, amPm);
  };
    ...
    <HStack align="start">
      <VStack align="start">
        <AppInput
          ...
          border="none"
        />
        <AppText fontSize="10px" fontWeight={400} color={neutral_400}>
          Hour
        </AppText>
      </VStack>
      <VStack align="start">
