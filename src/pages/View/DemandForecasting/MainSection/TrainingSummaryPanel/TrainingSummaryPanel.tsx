...
import AnchorTrainingSummary from './Tabs/AnchorTrainingSummary/AnchorTrainingSummary';
import { IDFView, dfViewSliceSelector } from 'state/pages/view/demandForecastView/dfViewPageState';
import { useSelector } from 'react-redux';

interface Props {
  onCloseTrainingSummary: () => void;
}

const TrainingSummaryPanel: FC<Props> = ({ onCloseTrainingSummary }) => {
  const dfViewState: IDFView = useSelector(dfViewSliceSelector);
  const isTrainingSummaryPanelOpen = dfViewState.isTrainingSummaryPanelOpen;

  return (
    <Modal onClose={() => {}} size={'full'} isOpen={isTrainingSummaryPanelOpen}>
      <ModalOverlay />
      <ModalContent bg="rgba(0, 0, 0, 0.60)" p="20px" w="full" h="full">
        <ModalCloseButton
          ...
          size="sm"
          zIndex={10}
          onClick={onCloseTrainingSummary}
        />
