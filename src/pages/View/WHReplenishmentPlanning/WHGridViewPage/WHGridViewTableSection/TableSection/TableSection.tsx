...
const TableSection: FC<Props> = () => {
  ...
  const tableHeaders = rplWhViewState.rplWhSkuExpandedDataList?.headers as TableHeader[];
  const tableDataList = rplWhViewState.rplWhSkuExpandedDataList?.list;
  const rplPlanTotalCount = rplWhViewState.rplWhPlanTotalCount;
  ...
  const totalCountsRplPlan = () => {
    return (
      <Tr p="0px" m="0px">
        ...
        >
          <AppText size="body2">
            Total ({rplWhViewState.rplWhSkuExpandedDataList?.totalCount} SKU-locations)
          </AppText>
        </td>
        <td 
          colSpan={2}
